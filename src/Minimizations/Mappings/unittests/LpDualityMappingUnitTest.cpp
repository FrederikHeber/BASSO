...
void LpDualityMappingUnitTest::twoNorm()
{
			...
			0.608996,0.567817,0.261505,-0.294843,-0.387682,-0.513624,-0.728372,-0.676635,-0.503763,-0.611381;
	{
		...
		const LpDualityMapping &J_p =
				static_cast<const LpDualityMapping &>(*SpaceX->getSpace()->getDualityMapping());
		...
		for (size_t i=0; i<10; ++i) {
			const Eigen::VectorXd compare = J_p(X.col(i));
...
void LpDualityMappingUnitTest::setTolerance()
{
	...
	CPPUNIT_ASSERT_EQUAL(BASSOTOLERANCE, J_p.getTolerance());
	const double value = 1e-1;
	J_p.setTolerance(value);
	CPPUNIT_ASSERT_EQUAL(value, J_p.getTolerance());
}

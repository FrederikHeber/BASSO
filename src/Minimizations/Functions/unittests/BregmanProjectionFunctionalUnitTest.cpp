...
void BregmanProjectionFunctionalUnitTest::setUp()
{
	...
	;
	MatrixVectorProduct = new OperationCounter<
...
void BregmanProjectionFunctionalUnitTest::oneNorm()
{
	...
	t << -0.091707,-0.897343,-0.127139,0.300883,-0.454444,0.967974,-0.262831,-0.668766,-0.670414,-0.455131,0.543059,0.865680,-0.300487,0.028029,-0.441100,0.019350,-0.775496,-0.908793,0.977006,-0.275997;
	X << -0.379750,-0.943336,-0.883381,0.807463,-0.697762,-0.066113,0.894920,-0.081799,0.728086,0.113930,-0.690921,0.420590,0.819499,-0.726720,0.284505,0.338650,-0.116910,0.517283,0.074392,0.520657;
	{
		...
		const BregmanProjectionFunctional d_p(lpdualnorm, J_1, power, *MatrixVectorProduct, *ScalarVectorProduct);
		for (size_t i=0; i<10; ++i) {
//			std::cout << "# " << i << ": Expecting " << expected(i) << " and got " << d_p(t.col(i),X.col(i),U,alpha) << ".\n";
			CPPUNIT_ASSERT( fabs( (expected(i) - d_p(t.col(i),X.col(i),U,alpha) )/expected(i) ) < 1e-4);
//			std::cout << "# " << i << ": Expecting " << expected_gradient.col(i).transpose() << " and got " << d_p.gradient(t.col(i),X.col(i),U,alpha).transpose() << ".\n";
			CPPUNIT_ASSERT( ((expected_gradient.col(i) - d_p.gradient(t.col(i),X.col(i),U,alpha) ).norm())/(expected_gradient.col(i)).norm() < 1e-4);
		}
	}
	{
...
//			std::cout << "# " << i << ": Expecting " << expected_gradient.col(i).transpose() << " and got " << d_p.gradient(t.col(i),X.col(i),U,alpha).transpose() << ".\n";
	...
	}
	{
		...
		for (size_t i=0; i<10; ++i) {
//			std::cout << "# " << i << ": Expecting " << expected(i) << " and got " << d_p(t.col(i),X.col(i),U,alpha) << ".\n";
			CPPUNIT_ASSERT( fabs( (expected(i) - d_p(t.col(i),X.col(i),U,alpha) )/expected(i) ) < 4e-4);
//			std::cout << "# " << i << ": Expecting " << expected_gradient.col(i).transpose() << " and got " << d_p.gradient(t.col(i),X.col(i),U,alpha).transpose() << ".\n";
			CPPUNIT_ASSERT( ((expected_gradient.col(i) - d_p.gradient(t.col(i),X.col(i),U,alpha) ).norm())/(expected_gradient.col(i)).norm() < 1e-4);
		}
	}
...
void BregmanProjectionFunctionalUnitTest::oneoneNorm()
{
	...
	X << 0.768586,-0.225475,0.470209,0.483057,-0.324979,-0.554573,0.030939,-0.352048,-0.103521,0.305334,-0.902129,-0.610977,0.396205,0.854957,0.562012,-0.658914,0.964865,-0.873672,0.717819,0.118345;
	{
		...
		const LpDualityMapping J_p(p, power);
		Eigen::VectorXd expected(10);
		...
		const BregmanProjectionFunctional d_p(lpdualnorm, J_p, power, *MatrixVectorProduct, *ScalarVectorProduct);
		for (size_t i=0; i<10; ++i) {
//			std::cout << "# " << i << ": Expecting " << expected(i) << " and got " << d_p(t.col(i),X.col(i),U,alpha) << ".\n";
			CPPUNIT_ASSERT( fabs( (expected(i) - d_p(t.col(i),X.col(i),U,alpha) ) ) < 1e-4);
//			std::cout << "# " << i << ": Expecting " << expected_gradient.col(i).transpose() << " and got " << d_p.gradient(t.col(i),X.col(i),U,alpha).transpose() << ".\n";
			CPPUNIT_ASSERT( (expected_gradient.col(i) - d_p.gradient(t.col(i),X.col(i),U,alpha) ).norm() < 1e-4);
		}
	}
	{
		const double power = 1.1;
...
{
	// due to large/small values we look at relative precision
	const LInfinityNorm lpdualnorm;
	Eigen::MatrixXd U(2,2);
	U <<   -0.37133, -0.95779, 0.40013, 0.93263;
	Eigen::VectorXd alpha(2,1);
	alpha << 0.014897, -0.568410;
	...
	X << 0.768586,-0.225475,0.470209,0.483057,-0.324979,-0.554573,0.030939,-0.352048,-0.103521,0.305334,-0.902129,-0.610977,0.396205,0.854957,0.562012,-0.658914,0.964865,-0.873672,0.717819,0.118345;
	{
		...
		const BregmanProjectionFunctional d_p(lpdualnorm, J_infty, power, *MatrixVectorProduct, *ScalarVectorProduct);
		for (size_t i=0; i<10; ++i) {
//			std::cout << "# " << i << ": Expecting " << expected(i) << " and got " << d_p(t.col(i),X.col(i),U,alpha) << ".\n";
			CPPUNIT_ASSERT( fabs( (expected(i) - d_p(t.col(i),X.col(i),U,alpha) )/expected(i) ) < 1e-4);
//			std::cout << "# " << i << ": Expecting " << expected_gradient.col(i).transpose() << " and got " << d_p.gradient(t.col(i),X.col(i),U,alpha).transpose() << ".\n";
...
//			std::cout << "# " << i << ": Expecting " << expected_gradient.col(i).transpose() << " and got " << d_p.gradient(t.col(i),X.col(i),U,alpha).transpose() << ".\n";
	...
	}
	{
		...
		for (size_t i=0; i<10; ++i) {
//			std::cout << "# " << i << ": Expecting " << expected(i) << " and got " << d_p(t.col(i),X.col(i),U,alpha) << ".\n";
			CPPUNIT_ASSERT( fabs( (expected(i) - d_p(t.col(i),X.col(i),U,alpha) )/expected(i) ) < 2e-4);
//			std::cout << "# " << i << ": Expecting " << expected_gradient.col(i).transpose() << " and got " << d_p.gradient(t.col(i),X.col(i),U,alpha).transpose() << ".\n";
			CPPUNIT_ASSERT( ((expected_gradient.col(i) - d_p.gradient(t.col(i),X.col(i),U,alpha) ).norm())/(expected_gradient.col(i)).norm() < 1e-4);
		}
	}

...
BOOST_PYTHON_MODULE(pyBasso)
{
	...
	class_<NormWrap, boost::shared_ptr<NormWrap>, boost::noncopyable>("Norm", no_init)
		.def_readonly("p", &Norm::getPvalue)
		.def_readonly("space", &Norm::getSpace)
		.def("__call__", &Norm::operator())
	    ;
	class_<MappingWrap, boost::shared_ptr<MappingWrap>, boost::noncopyable>("Mapping", no_init)
...
/*		.def("__call__", &Mapping::operator()) */
    ...
    class_<NormedSpace, boost::shared_ptr<NormedSpace> >("NormedSpace", no_init)
    ...
    const bool (SpaceElement::*SpaceElement_isZero_tolerance)(const double) const =
    		&SpaceElement::isZero;
    class_<SpaceElement, boost::shared_ptr<SpaceElement> >("SpaceElement", no_init)
	...
	class_<LinearMapping, bases<Mapping> >("LinearMapping", no_init)
		.def(self_ns::self * SpaceElement_ptr_t())
	;
//	class_<DualityMapping, bases<Mapping> >("DualityMapping", no_init)
//	;
    ...
    register_ptr_to_python< Norm_ptr_t >();
    register_ptr_to_python< Mapping_ptr_t >();
}
